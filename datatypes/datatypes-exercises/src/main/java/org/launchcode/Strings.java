package org.launchcode;

import java.util.Scanner;

public class Strings {
    public static void main(String[] arg){
        String alice = "Alice was beginning to get very tired of sitting by her sister on the bank, " +
                "and of having nothing to do: once or twice she had peeped into the book her sister " +
                "was reading, but it had no pictures or conversations in it, ‘and what is the use of " +
                "a book,’ thought Alice ‘without pictures or conversation?’";
        String aliceNew = alice.toLowerCase();

        Scanner input = new Scanner(System.in);
        System.out.println("Search Term: ");

        if (aliceNew.contains(input.next().toLowerCase())) {
            System.out.println("Term found");
            System.out.println("Index: " + aliceNew.indexOf(input.next()));
            System.out.println("Length: " + input.next().length());
        } else {
            System.out.println("Term not found");
        }
    }
}
